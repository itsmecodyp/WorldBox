...
namespace SimpleGUI.Menus {
    class GuiOther {
        ...
        //key: actor asset ID (unit_human, wolf) value: number to keep alive minimum
        public static Dictionary<string, int> racesToNaturallySpawn = new Dictionary<string, int>();

        //this is going to be absolutely hideous
        public static Dictionary<string, int> raceCountCurrent = new Dictionary<string, int>();
        ...
        {
            if(raceCountCurrent.ContainsKey(pStats) == false)
            {
                raceCountCurrent.Add(pStats, 0);
            }
            raceCountCurrent[pStats] += 1;
            Debug.Log("Current count for " + pStats + " is " + raceCountCurrent[pStats]);
        }
        ...
        public void MinimumNaturalSpawnUpdate()
        {
            if (naturalSpawnNewRaces)
            {
                foreach(string typeToSpawn in racesToNaturallySpawn.Keys)
                {
                    if (raceCountCurrent.ContainsKey(typeToSpawn))
                    {
                        if(raceCountCurrent[typeToSpawn] < racesToNaturallySpawn[typeToSpawn])
                        {
                            //3 attempts for random land tile, if none found just go with it
                            int attempt = 1;
                            WorldTile tile = World.world.tilesList.GetRandom();
                            while (tile.Type.liquid == false && attempt < 4)
                            {
                                tile = World.world.tilesList.GetRandom();
                                attempt++;
                            }
                            World.world.units.createNewUnit(typeToSpawn, tile, 0f);
                        }
                    }
                }
            }
